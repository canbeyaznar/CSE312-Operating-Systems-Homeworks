...
# 161044038
# SPIMOS_GTU_1.s 
# This kernel will load 4 different programs
# and it will enter an infinite loop until all the processes terminate.

.data

processID_msg: .asciiz "process ID -> "
currentPC_msg: .asciiz "currentPC -> "
endPC_msg: .asciiz "endPC -> "
...
main:
    ...
    ##  execve binary search

    la $a0, register1
    li $v0, 19
    la, $t6, binarysearch_name
    ...
    ##  execve linear search
    
    la $a0, register2
    li $v0, 19
    la, $t6, linearsearch_name
...
# run the loop and wait for the interrupt
loop:
    j loop
    j interrupt_handler
